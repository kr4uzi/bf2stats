...
class ServerModel
{
    ...
    private static ?Dictionary $Abbreviations = null;
...
SQL;
     ...
     */
    public function addServer($provider_id, $name, $ip, $port, $queryPort): int
    {
        /**
         * Check for valid IP address
         * @var iIPAddress $address
         */
     ...
     */
    public function deleteServers($ids): void
    {
        $count = count($ids);
     ...
     */
    public function queryServer(string $ip, $port)
    {
     ...
     */
    public function getArmyFromAbbreviation(&$name, &$flag): bool
    {
     ...
     * @return void
     */
    protected function addPlayerRank(array &$player)
    {
        $id = (int)$player['pid'];
        if ($id == 0)
        {
        ...
        else
        {
            ...
            if ($row = $rows->fetch())
            {
                $sn = strtolower(trim((string) $player['player']));
                $dn = strtolower(trim((string) $row['name']));
     ...
     * @param int $serverId
     */
    public function getServerChartData($serverId): array
    {
        // sanitize
