...
class Buffer
{
    ...
    public function getBuffer()
    {
        ...
        return substr((string) $this->data, $this->index);
    }
    ...
    public function read($length = 1)
    {
        ...
        $string = substr((string) $this->data, $this->index, $length);
        $this->index += $length;
    ...
    public function readLast()
    {
        ...
        $string = $this->data[strlen((string) $this->data) - 1];
        $this->data = substr((string) $this->data, 0, $len - 1);
        $this->length -= 1;
    ...
    public function lookAhead($length = 1)
    {
        ...
        return substr((string) $this->data, $this->index, $length);
    }
     ...
     * @param int $length
     */
    public function skip($length = 1): void
    {
    ...
    public function readString($delim = "\x00")
    {
        ...
        $len = strpos((string) $this->data, (string) $delim, min($this->index, $this->length));

        // If it is not found then return whole buffer
        if ($len === false) {
            return $this->read(strlen((string) $this->data) - $this->index);
        }
    ...
    public function readPascalString($offset = 0, $read_offset = false)
    {
        ...
        if ($read_offset) {
            ...
            return substr((string) $this->read($len), 0, $offset);
        }
    }
    ...
    public function readStringMulti($delims, &$delimfound = null)
    {
        ...
        foreach ($delims as $delim) {
            if ($p = strpos((string) $this->data, (string) $delim, min($this->index, $this->length))) {
                $pos[] = $p;
            }
        }
    ...
    public function readInt8()
    {
        ...
        $int = unpack('Cint', (string) $this->read(1));
    ...
    public function readInt16()
    {
        ...
        $int = unpack((string) $type, (string) $this->read(2));
    ...
    public function readInt32()
    {
        ...
        // Unpack the number
        $int = unpack((string) $type, (string) $this->read(4));
    ...
    public function readInt64()
    {
        ...
        if (version_compare(PHP_VERSION, '5.6.3') >= 0 && PHP_INT_SIZE == 8) {
            ...
            $int64 = unpack($type, (string) $this->read(8));
            $int = $int64['int'];
            unset($int64);
        } else {
            if ($this->number_type == self::NUMBER_TYPE_BIGENDIAN) {
    ...
    public function readFloat32()
    {
        ...
        $float = unpack('ffloat', (string) $string);
