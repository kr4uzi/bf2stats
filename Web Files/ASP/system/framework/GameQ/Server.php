...
class Server
{
    ...
    public function __construct(array $server_info = [])
    {
        ...
        try {
            ...
            $class = new ReflectionClass(
                sprintf('GameQ\\Protocols\\%s', ucfirst(strtolower((string) $server_info[self::SERVER_TYPE])))
            );
    ...
    protected function checkAndSetIpPort($ip_address)
    {
        ...
        // Test for IPv6
        if (substr_count((string) $ip_address, ':') > 1) {
            // See if we have a port, input should be in the format [::1]:27015 or similar
            if (strstr((string) $ip_address, ']:')) {
                // Explode to get port
                $server_addr = explode(':', (string) $ip_address);
            ...
            if (strstr((string) $ip_address, ':')) {
                [$this->ip, $this->port_client] = explode(':', (string) $ip_address);
