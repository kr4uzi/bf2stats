...
trait KeyManipulation
{
	...
	{
		$propagate = is_bool($propagate) ? $propagate : true;
		...
		if ($propagate && isset($this->mutates)) {
			...
			foreach ($this->mutates as $obj) {
				...
				if (in_array($prop, $obj->mutables)) {
					call_user_func([$obj, $prop], $this->{$prop});
				}
	 ...
	 * @return int
	 */
	protected function getAdjustedKeyLength(): float|int
	{
     ...
     */
    protected function __overloadMethods($method, $args): mixed
	{
		$_method = strtolower($method);

		if (in_array($_method, ['prefix', 'suffix'])) {
			return call_user_func_array([$this, 'affix'], array_merge([$_method], $args));
		}

		if ($_method === 'length') {
			return call_user_func_array([$this, 'length'], $args);
		}

		throw new BadMethodCallException(sprintf("Call to unknown method %s::%s()", static::class, $method));
	}
}
